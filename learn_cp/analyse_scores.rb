...
lines.each do |line|		
  if line.start_with?("space")
    ...
    if ARGV.length == 6
      get_score = %x[ ../learn_ga/bin/get_score -c #{constraint_type} -n #{nb_var} -d #{domain_size} -p #{parameter} -hi #{hamming_file} -f #{function} ]
    else
      get_score = %x[ ../learn_ga/bin/get_score -c #{constraint_type} -n #{nb_var} -d #{domain_size} -hi #{hamming_file} -f #{function} ]
    end
    ...
    scores[function] = scores[function] + occurence
  end
end

score_array = []
puts "Model \t\t\t\t occurence \t score"
functions.each do |model, score|
  puts "#{model} \t #{scores[model]} \t\t #{score}"
  for i in 0..scores[model]
...
puts "Most freq: #{most_freq_score}"
