...
double g( const vector< reference_wrapper<Variable> >& coeff, const vector<int>& vars, int start, int end, int var_max_value )
{
	...
	for( int i = start; i < end; ++i )
	{
		for( int k = 0; k < nb_freq / 2; ++k )
		{
			g_x += ( static_cast<double>( coeff[ ( ( i - start ) * nb_freq ) + 2*k ].get().get_value() - offset ) / 10 ) * cosine( vars[i], k, var_max_value );
			g_x += ( static_cast<double>( coeff[ ( ( i - start ) * nb_freq ) + 2*k + 1 ].get().get_value() - offset ) / 10 ) * sine( vars[i], k, var_max_value );
		}
	}
